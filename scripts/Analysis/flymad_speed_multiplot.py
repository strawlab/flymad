...
ax4.plot(expmean['align'].values, expmean['v'].values, 'b-', zorder=1, lw=3)
trans = mtransforms.blended_transform_factory(ax4.transData, ax4.transAxes)
ax4.fill_between(expmean['align'].values, (expmean['v'] + expstd['v']).values, (expmean['v'] - expstd['v']).values, color='b', alpha=0.1, zorder=2)
plt.axhline(y=0, color='k')
ax4.fill_between(expmean['align'].values, 0, 1, where=expmean['laser_state'].values>0, facecolor='Yellow', alpha=0.15, transform=trans, zorder=100)
ax4.plot(ctrlmean['align'].values, ctrlmean['v'].values, 'r-', zorder=1, lw=3)
ax4.fill_between(ctrlmean['align'].values, (ctrlmean['v'] + ctrlstd['v']).values, (ctrlmean['v'] - ctrlstd['v']).values, color='r', alpha=0.1, zorder=3)
